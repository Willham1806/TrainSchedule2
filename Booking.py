...
def booking_page():
    ...
    def display_feedback(container=None):
                ...
                if not two_caps and four_caps:
                    ...
                    with col2:
                        if st.button("❌ No, assign to 2-person carriage"):
                            ...
                            st.rerun()
                    st.markdown("---")
                    display_feedback(feedback_placeholder)
                    return
    ...
    if special_train_idx is not None:
        ...
        if not st.session_state.confirm_c45.get(key, False):
            st.warning(
                f"🚩 Is the group ok with carriages {', '.join(special_carriages)}"
            )
            col1, col2 = st.columns(2)
            with col1:
                if st.button(f"Yes"):
    ...
    # Try to preview assignment without committing changes (using a copy of the schedule)
    preview_schedule_copy = copy.deepcopy(schedule)
    preview_success, preview_schedule = assign_group(
        preview_schedule_copy, adults, toddlers, wheelchair_count, group_size, group_id,
    ...
    if preview_success:
        for train in preview_schedule:
            for carriage in train["carriages"]:
                if carriage.get("group_id") == group_id:
    ...
    # Format button label
    assign_label = "Assign Group"
    if train_time:
        assign_label += f" (to {train_time})"

    # Show button
    if st.button(assign_label) and group_size != 0:
        assigned, updated_schedule = assign_group(
            schedule, adults, toddlers, wheelchair_count, group_size, group_id,
            confirmed=False  # Full logic, but still non-committal for further confirmation
        )
        if assigned:
            save_schedule(updated_schedule)
            st.session_state.feedback = {"type": "success", "data": (updated_schedule, group_id)}
            st.session_state.reset_form = True
        else:
            st.session_state.feedback = {"type": "error", "data": "❌ No space on any upcoming train."}
        st.rerun()
